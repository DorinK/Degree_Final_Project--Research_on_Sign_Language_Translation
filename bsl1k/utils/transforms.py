...
def im_color_jitter(rgb, num_in_frames=1, thr=0.2, deterministic_jitter_val=None):
    ...
    if deterministic_jitter_val:
        assert (len(deterministic_jitter_val) == 3), "expected to be provided 3 fixed vals"
    ...
    if rgb.dim() == 3:
        rgb = im_to_video(rgb)
        assert (rgb.shape[1] == num_in_frames), "Unexpected number of input frames per clip"
        ...
        assert (rgb.shape[2] == num_in_frames), "Unexpected number of input frames per clip"
        rgb[:, 0].mul_(rjitter).clamp_(0, 1)
        rgb[:, 1].mul_(gjitter).clamp_(0, 1)
        rgb[:, 2].mul_(bjitter).clamp_(0, 1)
