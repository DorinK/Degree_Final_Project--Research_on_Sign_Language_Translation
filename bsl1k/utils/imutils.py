...
def resize_generic(img, oheight, owidth, interp="bilinear", is_flow=False):
    ...
    if chn == 1:
        ...
        # resized_img = scipy.misc.imresize(img, [oheight, owidth], interp=interp)
        # mode='F' gives an error for 3 channels
...
def text_on_image(img, txt=""):
    ...
    if w - 2 * x > img.shape[0]:
        font = ImageFont.truetype(font_name, int(img.shape[0] * img.shape[0] / (8 * (w - 2 * x))))
        w, h = font.getsize(txt)
