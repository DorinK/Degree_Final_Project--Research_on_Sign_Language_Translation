...
class FSR(nn.Module):
    ...
    def __init__(self, hidden_size, attn_size, n_layers, char_list, prior_gamma=1.0, ctc_type='builtin', iou_thr=0.5, n_top=50):
        ...
        self.int_to_char, self.char_to_int = dict([(i, c) for i, c in enumerate(char_list)]), dict(
            [(c, i) for i, c in enumerate(char_list)])
    ...
    def forward(self, Fs, fs_label, fs_mask, det_label, Ms=None, digit=True):
        ...
        fs_feats, fs_priors = Fs.new_zeros(len(prob_sizes), max(prob_sizes), feat_dim, h, w), Ms.new_zeros(
            len(prob_sizes), max(prob_sizes), h, w)
        ...
        labels, prob_sizes, label_sizes = torch.LongTensor(labels), torch.LongTensor(prob_sizes), torch.LongTensor(
            label_sizes)
        ...
        if self.ctc_type == 'warp':
            l = self.ctc_loss(logits.cpu(), labels.type(torch.int).cpu(), prob_sizes.type(torch.int).cpu(),
                              label_sizes.type(torch.int).cpu())
    ...
    def forward_reward(self, feats, priors, gt_coords, gt_chars, fs_mask, prop_coords, prop_scores):
        ...
        with torch.no_grad():
            ...
            for b in range(batch_size):
                ...
                batch_index, roi_coord, max_idx = (b + torch.zeros(mask.sum().item())).type_as(prop_coords), \
                                                  prop_coords[b][mask], max_idx[mask]
            ...
            roi_feats, roi_priors = feats.new_zeros(len(prob_sizes), max(prob_sizes), feats.size(2), feats.size(3),
                                                    feats.size(4)), priors.new_zeros(len(prob_sizes), max(prob_sizes),
                                                                                     feats.size(3), feats.size(4))
