...
class VideoData(tud.Dataset):
    ...
    def __init__(self, label_file, scp_file, pose_file, transform, max_num_wins=30, chunk_size=300, start_end=(0, 1e6),
                 bbox_file=None, image_scale=1, sigma=1, det_sample_rate=1, pose_sample_rate=1, fmap_wh=(12, 12)):
    ...
    def __getitem__(self, idx):
        ...
        chunk_id, rgb_video, opt_video, pose_json = self.chunk_ids[idx], self.rgb_videos[idx], self.opt_videos[idx], \
                                                    self.pose_data[idx]
        if pose_json != self.open_pose:
            self.open_pose = pose_json
            pose_data = json.load(open(pose_json, 'r'))
            self.handpose_buffer, self.bodypose_buffer, self.openpose_origin_wh = pose_data["hand_keypoints"], \
                                                                                  pose_data["body_keypoints"], \
                                                                                  pose_data["wh"]
        if rgb_video != self.open_rgb:
            self.open_rgb = rgb_video
            self.rgb_buffer = self._load_video(rgb_video, as_gray=False, scale=self.image_scale,
                                               bboxes=self.rgb_to_bbox[rgb_video])
        if opt_video != self.open_opt:
            self.open_opt = opt_video
            self.opt_buffer = self._load_video(opt_video, as_gray=True, scale=self.image_scale,
                                               bboxes=self.opt_to_bbox[opt_video])

        subid = self.subids[idx]
        bbox = self.bboxes[idx]

        opts_global, opts_local = self.opt_buffer['global'][self.chunk_size * subid: self.chunk_size * (subid + 1)][
                                  ::self.det_sample_rate], self.opt_buffer['local'][
                                                           self.chunk_size * subid: self.chunk_size * (subid + 1)][
                                                           ::self.det_sample_rate]
        imgs_global, imgs_local = self.rgb_buffer['global'][self.chunk_size * subid: self.chunk_size * (subid + 1)][
                                  ::self.det_sample_rate], self.rgb_buffer['local'][
                                                           self.chunk_size * subid: self.chunk_size * (subid + 1)][
                                                           ::self.det_sample_rate]
        ...
        if self.open_pose is not None:
            ...
            if bbox is None:
                ...
                heatmap_local, mask_local = self.get_heatmap(body_kps, hand_kps, w_img=self.openpose_origin_wh[0],
                                                             h_img=self.openpose_origin_wh[1], imgs=imgs_local,
                                                             bbox=bbox[::self.det_sample_rate])
        else:
            heatmap_local, mask_local, heatmap_global, mask_global = None, None, None, None

        sample = {'img_local': imgs_local, 'img_global': imgs_global, 'opt_local': opts_local,
                  'opt_global': opts_global, 'det_label': det_label, 'fs_label': fs_label, 'fs_mask': fs_mask,
                  'id': chunk_id, 'fs_id': fs_id, 'bbox': bbox, 'pose_heatmap_local': heatmap_local,
                  'pose_heatmap_global': heatmap_global, 'pose_mask_local': mask_local, 'pose_mask_global': mask_global,
                  'det_label_origin': det_label_origin}
    ...
    def collate_fn(self, data):
        ...
        sample['det_label'], sample['det_label_origin'] = torch.stack(
            [datum['det_label'] for datum in data]), torch.stack([datum['det_label_origin'] for datum in data])
        ...
        for key in ['_local', '_global']:
            if data[0]['pose_heatmap' + key] is not None:
                # pad
                B, lens, h, w, N = len(data), [datum['pose_heatmap' + key].shape[0] for datum in data], \
                                   data[0]['pose_heatmap' + key].shape[1], data[0]['pose_heatmap' + key].shape[2], \
                                   data[0]['pose_heatmap' + key].shape[3]
                hmap_pad, mask_pad = data[0]['pose_heatmap' + key].new_zeros(B, max(lens), h, w, N), data[0][
                    'pose_mask' + key].new_zeros(B, max(lens), N)
                for b in range(B):
    ...
    def _parse(self, label_file, scp_file, pose_dir, start_end, bbox_file):
        ...
        data_info = pickle.load(open(label_file, 'rb')) if label_file.split('.')[-1] == 'pkl' else json.load(
            open(label_file, 'r'))
