...
def generate_label_rpn(anchor_scales, feat_stride, vol_len, video_len, gt_labels, high_iou_thr=0.7, low_iou_thr=0.3,
                       neg_to_pos_ratio=3.0, min_neg_num=10):
    """
    gt_labels: [B, K, 3]
    return: [B, N], [B, N, 2]
    """
    ...
    anchor_cls_label, anchor_reg_label = -anchors.new_ones(batch_size, N).type(torch.LongTensor), -anchors.new_ones(
        batch_size, N, 2, dtype=torch.float32)
...
def generate_label_roi(rois, video_len, gt_labels, high_iou_thr=0.5, low_iou_thr=0.5, neg_to_pos_ratio=3.0,
                       min_neg_num=10):
    """
