...
class _TDCNN(nn.Module):
    ...
    def fwd_s1(self, sample, training=True, pose_on=False, fsr_on=False, pose_sample_rate=1, reward_on=True):
        ...
        # retrieve data
        video_data, opt_data, det_label, fs_label, fs_mask = sample['img_global'], sample['opt_global'], sample[
            'det_label'], sample['fs_label'], sample['fs_mask']
        ...
        # opt --> prior
        with torch.no_grad():
            prior_data = F.interpolate(opt_data.view(-1, opt_data.size(-2), opt_data.size(-1)).unsqueeze(dim=1),
                                       (base_feat_unflat.size(-2), base_feat_unflat.
                                        size(-1)), mode='bilinear').squeeze(dim=1).view(batch_size, L,
                                                                                        base_feat_unflat.size(-2),
                                                                                        base_feat_unflat.size(-1))
        ...
        if training:
            ...
            if fsr_on:
                ...
                fsr_cls_loss, fsr_pred, fsr_label, fsr_betas, fsr_probs = self.fsr(base_feat_unflat, fs_label, fs_mask,
                                                                                   det_label, Ms=prior_data,
                                                                                   digit=False)
            else:
            ...
            if reward_on:
                reward_loss = self.fsr.forward_reward(base_feat_unflat, prior_data, det_label, fs_label, fs_mask,
                                                      rois_coord, rois_score)
            else:
    ...
    def fwd_s2(self, sample, training=True, pose_on=False, fsr_on=False, pose_sample_rate=1, reward_on=True):
        ...
        # opt --> prior
        with torch.no_grad():
            prior_data_local, prior_data_global = self.make_prior(opt_data_local, base_feat_unflat.size(-2),
                base_feat_unflat.size(-1)), self.make_prior(opt_data_global, base_feat_unflat.size(-2), base_feat_unflat.size(-1))
        ...
        if training:
            ...
            if reward_on:
                reward_loss = self.fsr.forward_reward(base_feat_unflat[1], prior_data_local, det_label, fs_label,
                                                      fs_mask, rois_coord, rois_score)
            else:
                reward_loss = rpn_cls_loss.new_zeros(1)
    ...
    def make_prior(self, opt_data, h, w):
        ...
        prior_data = F.interpolate(opt_data.view(-1, opt_data.size(-2), opt_data.size(-1)).unsqueeze(dim=1), (h, w),
                                   mode='bilinear').squeeze(dim=1).view(batch_size, L, h, w)
        prior_data = prior_data / prior_data.max(dim=-1)[0].max(dim=-1)[0].view(batch_size, L, 1, 1).clamp_(min=1)
    ...
    def get_beta(self, sample):
        ...
        fs_mask[fs_mask == 0] = -1
        fsr_cls_loss, fsr_pred, fsr_label, fsr_betas, fsr_probs = self.fsr(base_feat_unflat, fs_label, fs_mask,
                                                                           det_label, Ms=prior_data, digit=False)
