...
def group_chunk(chunk_ids, output_rgb_path, output_opt_path, raw_rgb_path, raw_opt_path):
    ...
    for i, _ in enumerate(raw_video_paths):
        ...
        for j, chunk_id in enumerate(chunk_ids):
            ...
            in_imnames, out_imnames = [str(x).zfill(decimal) + suffix for x in range(start + 1, end + 1)], [
                str(x).zfill(decimal) + suffix for x in range(1, end - start + 1)]
        ...
        cmd = ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", list_fn, "-c:v", "libx264", "-pix_fmt", "yuv420p",
               "-crf", "20", output_path]
        print(" ".join(cmd))
...
def make_scp(label_file, data_dir, N_per_file=40):
    ...
    for video_id, chunk_ids in video_to_chunk_ids.items():
        for i in range(0, len(chunk_ids), N_per_file):
            data_fn = [os.path.join(data_dir, str(data_id) + '-rgb.mp4'),
                       os.path.join(data_dir, str(data_id) + '-opt.mp4')]
