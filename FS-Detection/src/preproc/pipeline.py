...
def main():
    ...
    raw_video, raw_pose, resize_video, opt_video, tmp_dir, loader_dir = os.path.join(root_dir,'raw-video'), os.path.join(
        root_dir, 'raw-pose'), os.path.join(root_dir, 'rgb-640x360'), os.path.join(root_dir,'opt-640x360'), os.path.join(
        root_dir, 'tmp'), os.path.join(root_dir, 'loader')
    ...
    if args.step == 1:
        ...
        for split, label_fn in zip(splits, label_fns):
            make_video(label_fn, os.path.join(loader_dir, 'video', split), rgb_dir=resize_video, opt_dir=opt_video,
                       **job_option)
    else:
