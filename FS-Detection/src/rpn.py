...
class RPN(nn.Module):
    ...
    def __init__(self, din, anchor_scales, feat_stride, neg_thr, pos_thr, nms_top_n, nms_thresh, min_size,
                 out_scores=False, device=torch.device('cuda')):
    ...
    def forward(self, base_feat, gt_chunks, video_len, training=True):
        ...
        prop_coord, prop_score = generate_proposal(self.anchor_scales, prob_vol, delta_vol, self.feat_stride,
                                                   self.nms_top_n, self.nms_thresh)
        if training:
            assert gt_chunks is not None
            cls_label, reg_label = generate_label_rpn(self.anchor_scales, self.feat_stride, prob_vol.size(-1),
                                                      video_len, gt_chunks, high_iou_thr=self.pos_thr,
                                                      low_iou_thr=self.neg_thr)
            prob_vol, delta_vol = prob_vol.view(batch_size, 2, -1).transpose(1, 2).contiguous(), delta_vol.view(
                batch_size, 2, -1).transpose(1, 2).contiguous()
