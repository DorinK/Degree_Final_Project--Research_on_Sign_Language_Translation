...
class Evaluator(object):
    ...
    def __init__(self, label_fn, scp_fn, model_fn):
        ...
        for line_id, line in enumerate(data_info):
            ...
            det_label, fsid, fs_label, fs_mask = np.array([[win[0], win[1]] for win in line['wins']]), line['fs_id'], \
                                                 line['fs_label'], np.array(line['fs_mask'])
            if len(fsid) > 0:
                det_label = det_label[fs_mask]
                idx_sorted = np.argsort(det_label[:, 0]).tolist()
                fsid, fs_label, det_label = [fsid[ix] for ix in idx_sorted], [fs_label[ix] for ix in idx_sorted], [
                    det_label[ix].tolist() for ix in idx_sorted]
...
def get_mAP(pred_pkl, stat_pkl, ler_thrs=[0.0], iou_thrs=[0.1], ordering=False):
    ...
    for ler_thr, iou_thr in zip(ler_thrs, iou_thrs):
        ...
        for num_roi_thr in num_roi_thrs:
            ...
            args = list(
                zip(y_pred, y_true, [ler_thr for _ in range(len(y_pred))], [iou_thr for _ in range(len(y_pred))],
                    [ordering for _ in range(len(y_pred))]))
