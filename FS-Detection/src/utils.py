...
def get_scale_range(base_size=8, scales=2 ** np.arange(3, 6)):
    ...
    ls = l * scales
    ls = ls[:, np.newaxis]
    scale_range = np.hstack((x_ctr - 0.5 * (ls - 1), x_ctr + 0.5 * (ls - 1))).astype(np.float32)
...
def mask_rpn_losses(masks_pred, masks_label):
    ...
    loss = F.binary_cross_entropy(masks_pred.view(batch_size, -1), masks_label, weight, size_average=False)
    loss /= weight.sum()
    return loss
