...
def wacc_list(references, hypotheses):  # TODO: Create wacc_list function.  V
    """
    Adjusting the wer_list function in order to calculate the WAcc (Word Accuracy Rate) metric.
    """
    total_error = total_del = total_ins = total_sub = total_ref_len = 0

    for r, h in zip(references, hypotheses):
        res = wacc_single(r=r, h=h)
        total_error += res["num_err"]
        total_del += res["num_del"]
        total_ins += res["num_ins"]
        total_sub += res["num_sub"]
    ...
    wacc = (1 - (total_error / total_ref_len)) * 100
    del_rate = (total_del / total_ref_len) * 100
    ins_rate = (total_ins / total_ref_len) * 100
    sub_rate = (total_sub / total_ref_len) * 100
...
def wacc_single(r, h):  # TODO: Create wacc_single function.    V
    """
    Adjusting the wer_list function in order to calculate the WAcc (Word Accuracy Rate) metric.
    """
    r = r.strip().split()
...
def edit_distance_wacc(r, h):  # TODO: Create edit_distance_wacc function.  V
    """
    Original Code from https://github.com/zszyellow/WER-in-python/blob/master/wer.py
    This function is to calculate the edit distance of reference sentence and the hypothesis sentence.
    Main algorithm used is dynamic programming.
...
def get_alignment_wacc(r, h, d):  # TODO: Create get_alignment_wacc function.   V
    """
    Original Code from https://github.com/zszyellow/WER-in-python/blob/master/wer.py
    This function is to get the list of steps in the process of dynamic programming.
